...
Future<void> main() async {
  ...
  proxy.connections.listen((connection) async {
    print(
        'Direct ${connection.type.name}: ${connection.address.address}:${connection.port} -> ${connection.desiredAddress.address}:${connection.port}');
    // Apply default handler
    await connection
        .redirect(ProxySettings(InternetAddress.loopbackIPv4, 1082));
  }).onError(print);

  redirectProxy.connections.listen((connection) async {
    print(
        'Redirected ${connection.type.name}: ${connection.address.address}:${connection.port} -> ${connection.desiredAddress.address}:${connection.port}');
